...
#Layers and denseblocks that can be pruned
...
for layer in prunable_layers:
    ...
    path = os.path.join("Layer_pruned", '30')
...
for layer in prunable_layers:
    ...
    accuracy = evaluate_accuracy(model, test_loader)
    path = os.path.join("Layer_pruned", '50')
    torch.save(model, os.path.join(
        path, f"{layer}_sparsity_{sparsity:.4%}_acc_{accuracy:.2f}.pt"))
...
for layer in prunable_layers:
    ...
    accuracy = evaluate_accuracy(model, test_loader)
    path = os.path.join("Layer_pruned", '80')
    torch.save(model, os.path.join(
        path, f"{layer}_sparsity_{sparsity:.4%}_acc_{accuracy:.2f}.pt"))
...
for layer in prunable_layers:
    ...
    accuracy = evaluate_accuracy(model, test_loader)
    path = os.path.join("Layer_pruned", '90')
    torch.save(model, os.path.join(
        path, f"{layer}_sparsity_{sparsity:.4%}_acc_{accuracy:.2f}.pt"))
